...
const SliderFixedRange: React.FC<SliderFixedRangeProps> = ({
  ...
  const sliderValue = useSharedValue<Point>({
    left: INIT_POINT,
    right: maximumValue,
  });
    ...
    transform: [
      {
        translateX: withTiming(
          rightProgress.value - actualThumbSize.width / 2,
          {duration: 1},
        ),
      },
    ],
  ...
  const animatedTrackStyle = useAnimatedStyle(() => {
    ...
    const transform = [
      {translateX: withTiming(leftProgress.value, {duration: 1})},
    ];
  ...
  const onPressPoint = useCallback(
    (point: number) => {
      ...
      const value = {
        minimum: isPointToLeft
          ? getSliderValue(curPoint)
          : getSliderValue(left),
        maximum: isPointToLeft
          ? getSliderValue(right)
          : getSliderValue(curPoint),
      };
      ...
      updateSlider(
        range * curPoint,
        curPoint,
        isPointToLeft ? ThumbPosition.left : ThumbPosition.right,
      );
      onValueChange(value);
    },
