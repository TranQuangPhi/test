  ...
  >({
    ...
    onActive: (event, ctx) => {
      ...
      // When sliding the thumb over the track's width
        ...
        const value =
          progressing / (trackWidth / (maximumValue - minimumValue));
        runOnJS(updateSlider)(progressing, value + minimumValue);
      }
    },
    onEnd: () => {
      const {range} = sliderInfo.value;
      let value = roundToValue
        ? sliderValue.value.toFixed(roundToValue)
        : sliderValue.value;
